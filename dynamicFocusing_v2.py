...
# TODO: Complete the training implementation i.e. evaluating and backpropagating losses - check to ensure gradient is being backpropagated
# TODO: 
...
def dyanamic_focusing_layer(input_fmap, Theta, specs, elemCoor, trainable=False):
    ...
    theta = tf.Variable(initial_value=theta_init, validate_shape=True, trainable=trainable) # theta is a variable, initialized at theta_init
    ...
    # fs = tf.constant(np.array(specs['IQSampleRate']).astype(np.float32)) # Original code
    ...
    # w0 = tf.constant(np.array(2.0 * np.pi * specs['DemodulationFrequency']).astype(np.float32)) # Original Code
    w0 = tf.constant(np.squeeze(2.0 * np.pi * specs['DemodulationFrequency']).astype(np.float32))
    ...
    delays_grid_clipped = tf.clip_by_value(delays_grid,clip_value_min=0.0, clip_value_max=dims[1]-1.0)
    ...
    out_fmap = bilinear_sampler(input_fmap, delays_grid_clipped) # Reminder: input_fmap is shape=(2, 652, 64, 140, 1), delays_grid_clipped is (652, 64, 140)
    ...
    cos_phi = tf.expand_dims(tf.cos(w0*(delays_grid_clipped-tt)),axis=3) 
    sin_phi = tf.expand_dims(tf.sin(w0*(delays_grid_clipped-tt)),axis=3)
...
if __name__=='__main__':
    ...
    input_fmap = tf.convert_to_tensor(img, dtype=tf.float32)
    ...
    # theta = np.array(specs['thetaRX']).astype(np.float32) # Extract specs['thetaRx'] and typecast it # Original code
    ...
    # elemCoor = tf.constant(np.array(elemCoor['elements_positions'][:, 0]).astype(np.float32)) # Second and third column are irrelevant - they're tossed. - Original Code
    elemCoor = tf.constant(np.squeeze(elemCoor['elements_positions'][:, 0]).astype(np.float32)) # Second and third column are irrelevant - they're tossed. - Original Code
    ...
    gradients = tape.gradient(loss, elemCoor)
    optimizer = tf.optimizers.Adam(0.1)
    optimizer.apply_gradients(zip(gradients, elemCoor)) # TODO: Apply the gradient update step... Currently bugging out!
    # %matplotlib inline
    IQ = BFfmap
    import matplotlib.pyplot as plt
    import matplotlib.image as mpimg
    # Separate I and Q components
    I_channel = np.array(IQ[0,:,:,:,0])
    Q_channel = np.array(IQ[1,:,:,:,0])
    ...
    # fs = np.array(specs['IQSampleRate'])     # Original code
    fs = np.squeeze(specs['IQSampleRate'])     
    t = (np.arange(input_img.shape[0], dtype=np.float32)) / fs # .shape[0] is the depth dimension = 652
    # w0 = (np.array(2.0 * np.pi * specs['DemodulationFrequency']).astype(np.float32)) # Original code
    w0 = (np.squeeze(2.0 * np.pi * specs['DemodulationFrequency']).astype(np.float32))
