...
def dynamic_focusing_layer(img_tensor, theta, specs, elemCoor, device, trainable=False):

    dims = img_tensor.shape # function to get the shape - returns [2, 652, 64, 140, 1]
    ...
    c = torch.from_numpy(np.squeeze(specs['SpeedOfSound']).astype(np.float32)).requires_grad_(requires_grad=False).to(device)  # 1540 m/s
    fs = torch.from_numpy(np.squeeze(specs['IQSampleRate']).astype(np.float32)).requires_grad_(requires_grad=False).to(device) # 3571428.5000 Hz
    ...
    ee, tt, ss = torch.meshgrid([elemCoor, t, theta]) # TODO: Check requires_grad properties here...
...
if __name__=='__main__':
    ...
    img_tensor = torch.from_numpy(img).to(device) # Still shares memory with img - modifying it will modify img
    ...
    # get BFed data
    BFfmap = dynamic_focusing_layer(img_tensor, theta, specs, elemCoor, device, trainable=trainable)
    ...
    if to_plot:
        ...
        output_img = scipy.interpolate.griddata((x_rx_input_vectorized, z_rx_input_vectorized), input_img.reshape(np.prod(input_img.shape[:])), (x_rx_output, z_rx_output), method = 'linear', fill_value=np.NaN)
        IQ_complex_sum = output_img/np.nanmax(abs(output_img))
        IQ_complex_sum_abs_logCompressed =  20*np.log10(abs(IQ_complex_sum))
        IQ_complex_sum_abs_logCompressed = np.nan_to_num(IQ_complex_sum_abs_logCompressed)
