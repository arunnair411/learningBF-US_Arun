...
def load_data():
    data_dir = 'dynamicFocusing_PyTorch_SinglePWSim'
    file_name = '007415.mat'
    ...
    labels_dict['c'] = curr_label['c'].item().item() # Want to remove all the extraneous array([]) objects and retrieve only the value out for sanity
    ...
    probe_dict['N_elements'] = 128
    probe_dict['pitch']      = 0.3e-3
    ...
    # Get the lowpass filter weights and lag estimate from MATLAB - you calculate those temrs in matlab, and just read it from a mat file here
    lpf_and_lag = loadmat(os.path.join(data_dir, 'matlab_deets.mat'))
    probe_dict['lpf_weights'] = np.squeeze(lpf_and_lag['filter_weights'])
    probe_dict['lag'] = lpf_and_lag['lag'].item()

    data_dict = {}
    data_dict['v_2']        = mat_data['v_2']
    data_dict['t_2']        = mat_data['t_2'].item()
