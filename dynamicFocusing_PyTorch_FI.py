...
def load_data():
    data_dir = 'dynamicFocusing_PyTorch_FI'
    file_name = '20180524-143131/213_layer0_idx213_BDATA_RF.mat'
    seq_file_name = '20180524-143131/Sequence.mat'
    ...
    probe_dict['scCoor']     = probe_dict['elemCoor'][0]+ np.arange(0, probe_dict['N_elements'], 0.5) * probe_dict['pitch']
    ...
    data_dict = {}
    # WHOLE THING BELOW IS MY ATTEMPT AT THE PROCESSING PIPELINE - NOT WORTH IT! Just do the first bit in USTB... 
    # NOTE: You have to change the hardcoded offset to 0 in read_FI_linear though
    # data_dict['v_2'] = np.zeros((mat_data['AlignedSampleNum'].item(), probe_dict['N_elements'], mat_data['ScanlineNum'].item())).astype(np.float32)
    
    # curr_mat_data_key = "AdcData_scanline{0:03d}_thi0_sa0".format(0)
    # col_idx = [x for x in range(-32,32)]    
    # col_idx_trimmed = [x for x in range(-32,32) if (x>=0 and x<probe_dict['N_elements'])]
    # data_placed = np.zeros((mat_data['AlignedSampleNum'].item(), probe_dict['N_elements'])).astype(np.float32)
    # if min(col_idx)<0:        
    #     temp_data = np.reshape(np.swapaxes(mat_data[curr_mat_data_key],0,1), (mat_data[curr_mat_data_key].shape[1], mat_data[curr_mat_data_key].shape[0] * mat_data[curr_mat_data_key].shape[2]), order='F').astype('float32')
    #     data_placed[:,0:64] = temp_data
    # elif max(col_idx)>=probe_dict['N_elements']:
    #     temp_data = np.reshape(np.swapaxes(mat_data[curr_mat_data_key],0,1), (mat_data[curr_mat_data_key].shape[1], mat_data[curr_mat_data_key].shape[0] * mat_data[curr_mat_data_key].shape[2]), order='F').astype('float32')
    #     data_placed[:,64:128] = temp_data        
    # else:
    #     temp_data = np.reshape(np.swapaxes(mat_data[curr_mat_data_key],0,1), (mat_data[curr_mat_data_key].shape[1], mat_data[curr_mat_data_key].shape[0] * mat_data[curr_mat_data_key].shape[2]), order='F').astype('float32')
    #     data_placed[:,col_idx] = temp_data
    # data_dict['v_2'][:, col_idx_trimmed, 0] = data_placed[:, col_idx_trimmed]
    # for jdx in range(1,256):
    #     curr_mat_data_key = "AdcData_scanline{0:03d}_thi0_sa0".format(jdx)
    #     col_idx = [x+(jdx-1)//2 for x in range(-32,32)]
    #     col_idx_trimmed = [x+(jdx-1)//2 for x in range(-32,32) if (x+(jdx-1)//2>=0 and x+(jdx-1)//2<probe_dict['N_elements'])]
    #     data_placed = np.zeros((mat_data['AlignedSampleNum'].item(), probe_dict['N_elements'])).astype(np.float32)
    #     if min(col_idx)<0:
    #         temp_data = np.reshape(np.swapaxes(mat_data[curr_mat_data_key],0,1), (mat_data[curr_mat_data_key].shape[1], mat_data[curr_mat_data_key].shape[0] * mat_data[curr_mat_data_key].shape[2]), order='F').astype('float32')
    #         data_placed[:,0:64] = temp_data
    ...
    #         temp_data = np.reshape(np.swapaxes(mat_data[curr_mat_data_key],0,1), (mat_data[curr_mat_data_key].shape[1], mat_data[curr_mat_data_key].shape[0] * mat_data[curr_mat_data_key].shape[2]), order='F').astype('float32')
    #         data_placed[:,col_idx] = temp_data
    #     data_dict['v_2'][:, col_idx_trimmed, jdx] = data_placed[:, col_idx_trimmed]
    ...
    # # TODO: Hardcoded offset which is different for each scanline is not computed or compensated for...
    ...
    data_dict['t_2']        = 0 # There is already some resampling and time compensation happening in UST when it pre-processes the data, so don't need it here
    data_dict['min_sample'] = data_dict['t_2']*probe_dict['fs']
    data_dict['t_in']       = (np.arange(data_dict['v_2'].shape[0]) + data_dict['min_sample'])/probe_dict['fs'] - probe_dict['lag']*probe_dict['dt']
    data_dict['min_sample_wlag'] = data_dict['t_2'] - probe_dict['lag']*probe_dict['dt']
    data_dict['z_in']       = data_dict['t_in']*labels_dict['c']/2    
    ...
    data_dict['v_2_downmixed'] = data_dict['v_2'] * np.tile(np.reshape(data_dict['complex_sinusoid'], (len(data_dict['complex_sinusoid']), 1, 1)), (1, probe_dict['N_elements'], mat_data['ScanlineNum'].item()))
    ## Step 2: Low pass filter the downmixed data 
    data_dict['v_2_downmixed_lpf'] = np.zeros_like(data_dict['v_2_downmixed'])
    for jdx in range(data_dict['v_2_downmixed'].shape[2]):
        data_dict['v_2_downmixed_lpf'][:,:,jdx] = convolve2d(data_dict['v_2_downmixed'][:,:,jdx], np.reshape(probe_dict['lpf_weights'], (len(probe_dict['lpf_weights']), 1)), mode='same')
    # Issue With Below - complex data not supported...
    # data_dict['v_2_downmixed_lpf'] = convolve(data_dict['v_2_downmixed'], np.reshape(probe_dict['lpf_weights'], (len(probe_dict['lpf_weights']), 1, 1)), mode='constant')
...
def dynamic_focusing_layer(img_tensor, data_dict, probe_dict, labels_dict, elemCoor, scCoor, device, trainable=False):
    ...
    ee, tt, ss = torch.meshgrid([elemCoor, t, scCoor]) # TODO: Check requires_grad properties here...
...
if __name__=='__main__':
    ...
    dims = [data_dict['v_2_downmixed_lpf_resampled'].shape[0], data_dict['v_2_downmixed_lpf_resampled'].shape[1], data_dict['v_2_downmixed_lpf_resampled'].shape[2]]
    ...
    # I = np.expand_dims(np.expand_dims(I,0),4) # Change I's dimensions from a [1536, 128, 256] array to a [1, 1536, 128, 256] array
    I = np.expand_dims(I,0) # Change I's dimensions from a [1536, 128] array to a [1, 1536, 128, 256] array
    ...
    Q = np.expand_dims(Q,0) # Change Q's dimensions from a [1536, 128, 256] array to a [1, 1536, 128, 256] array
    img = np.concatenate((I, Q), axis=0) # Concatenate the I and Q channels on the first axis to yield a [2,1536,128, 256] array
    ...
    img_tensor = torch.from_numpy(img[:, :, :, :, np.newaxis]).to(device) # Still shares memory with img - modifying it will modify img
    ...
    scCoor   = torch.from_numpy(probe_dict['scCoor'].astype(np.float32)).requires_grad_(requires_grad=trainable).to(device) 

    # specs = specs['specs']

    # get BFed data
    BFfmap = dynamic_focusing_layer(img_tensor, data_dict, probe_dict, labels_dict, elemCoor, scCoor, device, trainable=trainable)
